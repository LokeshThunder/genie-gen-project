...
export default function LiquidEther({
  ...
  // Time-aware color logic
  const getTimeColors = () => {
    const hour = new Date().getHours();
    if (hour >= 6 && hour < 18) {
      // Day: Mint, Sky Blue, White
      return ['#00F5D4', '#00BBFF', '#FFFFFF'];
    } else {
      // Night: Neon Purple, Deep Blue, Magenta
      return ['#9D50BB', '#6E48AA', '#FF0080'];
    }
  };

  const activeColors = colors && colors.length > 0 ? colors : getTimeColors();
