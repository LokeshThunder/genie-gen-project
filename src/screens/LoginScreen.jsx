...
const LoginScreen = ({ onLogin }) => {
  ...
  const handleSendOTP = async () => {
    ...
    setErrorMessage(null);
    try {
      // Ensure number starts with +91 or whatever is required
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        {errorMessage && (
          <div style={{ background: "#FEE2E2", border: "1px solid #FCA5A5", color: "#B91C1C", padding: "12px", borderRadius: 12, fontSize: 11, marginBottom: 16, lineHeight: 1.4 }}>
            ...
            {errorMessage.includes("undefined") && (
              <div style={{ marginTop: 8, color: "#1E1B4B", fontWeight: 700 }}>
                💡 Tip: Use test number `1111111111` to bypass this for testing.
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
          <div 
            ...
            style={{ flex: 1, border: role === 'worker' ? "2px solid #5B3FC8" : "1.5px solid #eee", background: role === 'worker' ? "#F5F3FF" : "#fff", borderRadius: 14, padding: "11px 0", textAlign: "center", fontWeight: 700, color: role === 'worker' ? "#5B3FC8" : "#bbb", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, cursor: "pointer", transition: "all 0.2s" }}
          >
            👤 WORKER
          </div>
          <div 
            ...
            className="tap-effect"
            style={{ flex: 1, border: role === 'admin' ? "2px solid #5B3FC8" : "1.5px solid #eee", background: role === 'admin' ? "#F5F3FF" : "#fff", borderRadius: 14, padding: "11px 0", textAlign: "center", fontWeight: 700, color: role === 'admin' ? "#5B3FC8" : "#bbb", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, cursor: "pointer", transition: "all 0.2s" }}
          >
            💼 COMPANY
          </div>
        </div>
